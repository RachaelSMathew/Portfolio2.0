      ...
      (transformProjects !== 1.6 && transformBooks!==1.8) && <div onClick={()=>{transformJobs!==1.3 && setTransformJobs(1.3)}}  onMouseLeave={() => {if(transformJobs===1.1) setTransformJobs(1)}} onMouseEnter={() => {if(transformJobs==1) setTransformJobs(1.1)}}  style={{cursor: transformJobs !== 1.3 ? "pointer":"default", position: "relative", transition: "1.5s", transform: `scale(${transformJobs})`}}>
        ...
        <div onMouseLeave={() => transformJobs!==1.3&&setOpacityImageJobs(0)} onMouseEnter={() => transformJobs!==1.3&&setOpacityImageJobs(0.6)} style={{position: "absolute", top: 100, left: 115, height: "62%", width: "54%", transition: ".5s ease", borderRadius: "210px /290px", backgroundColor: `  rgba(178,157,103,${opacityImageJobs})`}}>
          ...
          <p ref={highlightTextRef} style={{position: "absolute",transition: ".5s", left: 30, top: 30, right: 30, color: "black" }}>
            ...
            <h1 style={{fontSize :"20px", color: scrollJobs>=14.44?"black":"#574b0f", transition: "all .5s ease-out", textAlign: "center", fontWeight: "bold"}}>Flux Marine, Front End Web Developer</h1>
            <h1 style={{color: scrollJobs>=44.44?"black":"#574b0f", transition: "all .5s ease-out", fontSize:"15px"}}>Feb 2024 - June 2024</h1>
            <Image style={{position: "absolute", top: 10, transition: "all 1.5s ease-out", opacity: opacityFlux}} src={flux} width={460} alt={""}    />
            <h1 style={{fontSize :"15px", width: "90%", position: "relative", color: scrollJobs>=60.44?"black":"#574b0f", transition: "all .5s ease-out"}} >&nbsp;&nbsp;&nbsp;&nbsp;Developed front end of a progressive web app using React Native that allows users to track their boat’s location, past trips, speed, battery, voltage, and other metrics.</h1>
            <h1 style={{fontSize :"20px", color: scrollJobs>=145.44?"black":"#574b0f", transition: "all .5s ease-out", textAlign: "center", fontWeight: "bold"}}>GameChanger, iOS Developer Intern<br/><h1 style={{fontSize:"15px", textAlign: "left", color: scrollJobs>=175.44?"black":"#574b0f", transition: "all .5s ease-out",}}>June 2023 - Aug 2023</h1></h1>
            <Image style={{position: "absolute", top: 270, right: 40, transition: "all 2s ease-out", opacity: !firstScroll ? 0.5 : scrollJobs>=215.44?0.5:0}} src={gameChanger} width={260} alt={""}    />
            <h1 style={{fontSize :"15px", left: 15, position: "relative", color: scrollJobs>=215.44?"black":"#574b0f", transition: "all .5s ease-out"}} >&nbsp;&nbsp;&nbsp;&nbsp;Enhanced GameChanger app by adding linkable buttons using RxSwift, replacing directional icons as PDF files using SnapKit, adding/editing "help & feedback" buttons/snackbars, and realigning misplaced icons.</h1>
            <h1 style={{fontSize :"20px", textAlign: "center", fontWeight: "bold", color: scrollJobs>=335.44?"black":"#574b0f", transition: "all .5s ease-out"}}>iRobot, Software DevOps Co-op<br/><h1 style={{fontSize:"15px", textAlign: "left", color: scrollJobs>=355.44?"black":"#574b0f", transition: "all .5s ease-out"}}>June 2022 - Dec 2022</h1></h1>
            ...
            <h1 style={{fontSize :"15px", position: "relative", fontWeight: "bold", color: scrollJobs>=397.44?"black":"#574b0f", transition: "all .5s ease-out"}}>&nbsp;&nbsp;&nbsp;&nbsp;Designed Jenkins pipelines using Groovy and EC2 instances and executed tasks in a Docker container using an ECR image to automate tasks such as configuring repositories, uploading build files to Artifactory, and syncing files.</h1>
          </p>
          </div>}
        </div>
      </div>
