...
export default function NavBar() {
    ...
    return (
        <view style={{alignItems: "center", minHeight: changeNav ? "20px":"100%", zIndex: 2, minWidth: changeNav ? "100%":"20px", top: changeNav ? null:0, bottom:changeNav ? 0:null, right: 0, display: "flex", position: "fixed", justifyContent: "space-between", flexDirection:  changeNav ? 'row': "column"}}>
            <Image onMouseLeave={() => setGray("grayscale(0%)")} onMouseEnter={() => setGray("grayscale(100%)")} src={profilePic} style={{filter: gray, marginTop: -10, cursor: "pointer", transition: "1s"}} height={windowSize[1] < 600 ? 50:70} alt={""}    />
            ...
            <Image onMouseLeave={() => setGray("grayscale(0%)")} onMouseEnter={() => setGray("grayscale(100%)")} src={profilePic} style={{filter: gray, cursor: "pointer", transition: "1s"}} height={windowSize[1] < 600 ? 50:70} alt={""}    />
        </view>
    )
}
